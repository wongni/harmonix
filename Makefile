...
# Create a patch file that can be used to apply Harmonix customizations to a freshly installed
# Backstage. You need to have 2 copies of Backstage to create the patch. The patch file will
# be generated by comparing the 2 copies and outputting the differences. The first copy should
# be under the "backstage" directory and it should be configured with Harmonix customizations.
# The second copy should be a freshly downloaded backstage with no changes whatsoever.  It
# should be under the "backstage-pure" directory. You should delete the pure version after you 
# create the patch. If you need to generate the patch again, download a new pure version, which
# will ensure you have the latest Backstage patch version.
create-harmonix-patch:
	./build-script/create-backstage-harmonix-patch.sh
